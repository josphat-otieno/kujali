...
export class BudgetTableComponent implements OnChanges {
  ...
  // Input now receives signal value directly (not observable)
  ...
  private readonly _budgetsSignal = signal<{ overview: BudgetRecord[], budgets: any[] }>({ overview: [], budgets: [] });
  dataSource = new MatTableDataSource();
  displayedColumns: string[] = ['name', 'status', 'startYear', 'duration', 'actions'];
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild('sort', { static: true }) sort: MatSort;
  overviewBudgets: BudgetRecord[] = [];
  // Angular 15 compatible: Use OnChanges to detect input changes
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['budgets'] && changes['budgets'].currentValue) {
      const budgetsData = changes['budgets'].currentValue;
      this._budgetsSignal.set(budgetsData);
